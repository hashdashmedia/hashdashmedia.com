...
export function Video({ videoId, fallbackImg }: Readonly<VideoProps>) {
  const [playVideo, setPlayVideo] = useState(false);

  // If no videoId, just show fallback
  if (!videoId && fallbackImg) {
    return (
      <Container>
        <div className="relative w-full h-[500px] max-w-4xl mx-auto overflow-hidden lg:mb-20 rounded-2xl">
          <Image
            src={fallbackImg}
            alt="Demo Preview"
            fill
